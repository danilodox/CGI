...
public class Abertura {
    ...
    public Abertura(int[][] imagem, int width, int height, int times){
        this.imagem = imagem;
        this.width = width;
        this.height = height;
        this.times = times;
    ...
    public BufferedImage abertura(BufferedImage img, int times, boolean[][] kernel) {
        BufferedImage c = new Dilatar().dilate(new Erosao().erode(img, times, kernel), times, kernel);
        int[][] aux = convertToMatriz(c.getRaster());
        return Normalizacao.matrizToBufferedImage(aux, 255);
    }
    
    public BufferedImage run(){
        int[][] img1 = convertToMatriz((new Erosao(imagem, width, height, times).run()).getRaster());
        BufferedImage out = new Dilatar(img1, width, height, times).run();
